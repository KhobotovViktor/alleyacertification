...
export default function BranchesPage() {
  ...
  const generateGASScript = () => {
    ...
    return `// --- Alleya Feedback Service Automation Script ---
...
function syncService(branchId, service, url, apiUrl, apiKey) {
  try {
    ...
    if (service === "yandex") {
       const rMatch = html.match(/rating-text">([\\d,.]+)/) || html.match(/class="Rating-Value">([\\d,.]+)/);
       const cMatch = html.match(/aria-label="(\\d+)\\s+оцен/) || html.match(/class="Rating-Count">[^<]*?(\\d+)/);
       if (rMatch) rating = parseFloat(rMatch[1].replace(',', '.'));
       if (cMatch) count = parseInt(cMatch[1]);
       
    } else if (service === "2gis") {
       const rMatch = html.match(/class="_y10azs">([\\d.]+)/);
       const cMatch = html.match(/class="_jspzdm">(\\d+)\\s+оцен/);
       if (rMatch) rating = parseFloat(rMatch[1]);
       if (cMatch) count = parseInt(cMatch[1]);
       
    } else if (service === "google") {
       const rMatch = html.match(/<span aria-hidden="true">([0-5][.,]\\d)<\\/span>/);
       const cMatch = html.match(/aria-label="([\\d\\s,]+)\\s+(?:reviews|отзыв)"/);
       if (rMatch) rating = parseFloat(rMatch[1].replace(',', '.'));
